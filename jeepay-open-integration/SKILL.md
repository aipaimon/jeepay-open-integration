...
> ⚠️ 本技能执行前，会自动检测并提示用户配置所需权限。

**执行流程**：

1. 读取当前项目下的 `.claude/settings.local.json`，检查是否已包含所需权限。
2. 如已配置：直接继续。
3. 如未配置：**询问用户是否同意自动添加权限**，用户确认后自动写入。
...
**自动配置内容**（用户确认后写入 `.claude/settings.local.json`）：
...
- [6] 禁止编造配置、假数据；禁止绕过确认点
- [7] 禁止合理化跳过确认（如"用户可能希望快速完成"）
- [8] 禁止假数据与占位符冒充
...
> Skill 版本：1.0.0 | 适用版本：Jeepay 开源版 | SDK 版本：详见 [SDK下载文档](https://doc.jeequan.com/#/integrate/open/api/116)
